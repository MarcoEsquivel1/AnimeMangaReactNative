...
// @ts-ignore
export const MangaScreen: FC<StackScreenProps<AppStackScreenProps, "Manga">> = observer(
  function MangaScreen(props) {
        ...
        <ScrollView className="bg-black" style={{ flex: 1 }}>
          <View className="flex-1 bg-withe" style={{ height: 470 }}>
            <View className="h-full">
              ...
              <Animated.View entering={FadeInDown.delay(300)} exiting={FadeOut}>
                <SharedElement
                  id={`item.${manga.id}.photo2`}
          ...
          <View>
            ...
            <FlatList
              ...
              data={manga.chapters}
              renderItem={({ item }) =>
                item == null ? null : item.canonicalTitle != null ? (
                  <ImageBackground
                    source={
                      item.thumbnail != null && item.thumbnail.original != null
                        ? { uri: item.thumbnail.original }
                        : require("../../assets/images/error.png")
                    }
                    style={styles.image2}
                    imageStyle={{ borderRadius: 24 }}
                  >
                    <View className="flex-col p-2 backdrop-blur-lg bg-transparent/50 rounded-b-3xl h-32 w-32">
                      <View className="h-full">
                        <Text
                          numberOfLines={2}
                          ellipsizeMode="tail"
                        ...
                        >
                          {item.canonicalTitle != null ? item.canonicalTitle : "No encontrado"}
                        </Text>
                        <Text className="text-white font-semibold absolute bottom-0">
                          Capitulo: {item.number != null ? item.number : null}
                        </Text>
                      </View>
                    </View>
                  </ImageBackground>
          ...
          <View>
            ...
            <FlatList
              ...
              data={characters}
              renderItem={({ item }) =>
                item == null ? null : item.canonicalName != null ? (
                  <ImageBackground
                    source={
                      item.image != null && item.image.original != null
                    ...
                    <View className="flex-col p-2 backdrop-blur-lg bg-transparent/50 rounded-b-3xl h-32 w-32">
                      <View className="h-full">
                        <Text
                          ...
                          className="text-white font-semibold"
                        >
                          {item.canonicalName != null ? item.canonicalName : "No encontrado"}
                        </Text>
                      </View>
                    </View>
                  </ImageBackground>
